    ...
    if (settings.async) {
      ...
      var responseHandler = function responseHandler(resp) {
        ...
        response.on("end", function () {
          self.responseXML = html2dom(self.responseText, self.responseType);
          parseMissingJson(self);
          if (sendFlag) {
      ...
      var execString =
        ...
        "fs.writeFileSync('" +
        contentFile +
        "', JSON.stringify({err: null, data: {statusCode: response.statusCode, headers: response.headers, text: responseText, xml: responseXML}}), 'utf8');" +
        "fs.unlinkSync('" +
        syncFile +
        "');" +
        "});" +
 ...
 */
export function html2dom(html, responseType) {
...
export function parseMissingJson(request) {
  if (
